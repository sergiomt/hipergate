...
<%@ include file="../../methods/dbbind.jsp" %><%@ include file="../../methods/cookies.jspf" %><%@ include file="../../methods/authusrs.jspf" %><%
  ...
  try {
    oConn = GlobalDBBind.getConnection("bloglines_login");  
  
    oRec.load(oConn,request.getParameter("gu_pwd"));

    oConn.close("bloglines_login");
...
<HEAD>
  ...
  <TITLE>hipergate : Bloglines Autologin Form</TITLE>
  <SCRIPT TYPE="text/javascript">
    function postBlogLines() {
    	var frm = document.forms[0];
      frm.email.value="<%=oRec.getValueOf("email")%>";
      frm.password.value="<%=oRec.getValueOf("pwd")%>";
			frm.submit();
    }
  </SCRIPT>
</HEAD>
<BODY onLoad="postBlogLines()">
  <FORM METHOD="post" ACTION="http://www.bloglines.com/login">
    <INPUT TYPE="hidden" NAME="r" VALUE="/myblogs">

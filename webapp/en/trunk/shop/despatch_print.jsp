<%@ page import="java.util.Properties,java.math.BigDecimal,java.io.File,java.io.IOException,java.io.ByteArrayInputStream,java.io.ByteArrayOutputStream,java.net.URLDecoder,java.sql.SQLException,com.knowgate.jdc.JDCConnection,com.knowgate.dataobjs.*,com.knowgate.acl.*,com.knowgate.hipergate.DespatchAdvice,com.knowgate.misc.Environment,com.knowgate.dfs.FileSystem,com.knowgate.dataxslt.StylesheetCache" language="java" session="false" contentType="text/html;charset=UTF-8" %><%@ include file="../methods/dbbind.jsp" %><%@ include file="../methods/cookies.jspf" %><%@ include file="../methods/authusrs.jspf" %><%@ include file="../methods/nullif.jspf" %><%@ include file="templates.jspf" %><%
  ...
  DespatchAdvice oInvc = null;

  try {
    oConn = GlobalDBBind.getConnection("despatch_print");    
    oInvc = new DespatchAdvice();
    oInvc.load(oConn, new Object[]{gu_despatch});
    oConn.close("despatch_print");
  }
  ...
  ByteArrayInputStream oByInStrm = new ByteArrayInputStream(oInvc.toXML().getBytes("UTF-8"));
  
  String sXslPath = getXSLTemplatePath(sStorage, sIdDomain, sGuWorkArea, oInvc.getString(DB.gu_shop), "despatch.xsl");
  
  if (sXslPath==null)
    throw new java.io.FileNotFoundException("Cannot find despatch.xsl template file at storage " + sStorage + " for WorkArea " + sGuWorkArea + " of Domain " + sIdDomain + " nor at " + sStorage+"xslt"+sSep+"templates"+sSep+"Shop"+sSep+"despatch.xsl");

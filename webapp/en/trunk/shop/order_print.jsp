<%@ page import="java.util.Properties,java.math.BigDecimal,java.io.File,java.io.IOException,java.io.ByteArrayInputStream,java.io.ByteArrayOutputStream,java.net.URLDecoder,java.sql.SQLException,com.knowgate.jdc.JDCConnection,com.knowgate.dataobjs.*,com.knowgate.acl.*,com.knowgate.hipergate.Order,com.knowgate.misc.Environment,com.knowgate.dfs.FileSystem,com.knowgate.dataxslt.StylesheetCache" language="java" session="false" contentType="text/html;charset=UTF-8" %><%@ include file="../methods/dbbind.jsp" %><%@ include file="../methods/cookies.jspf" %><%@ include file="../methods/authusrs.jspf" %><%@ include file="../methods/nullif.jspf" %><%@ include file="templates.jspf" %><%
  ...
  try {
    oConn = GlobalDBBind.getConnection("order_print");    
    oInvc = new Order();
    oInvc.load(oConn, new Object[]{gu_order});
    oConn.close("order_print");
  }
  ...
  String sXslPath = getXSLTemplatePath(sStorage, sIdDomain, sGuWorkArea, oInvc.getString(DB.gu_shop), "order.xsl");
  
  if (sXslPath==null)
    throw new java.io.FileNotFoundException("Cannot find order.xsl template file at storage " + sStorage + " for WorkArea " + sGuWorkArea + " of Domain " + sIdDomain + " nor at " + sStorage+"xslt"+sSep+"templates"+sSep+"Shop"+sSep+"despatch.xsl");

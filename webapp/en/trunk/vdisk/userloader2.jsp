...
<%@ include file="../methods/dbbind.jsp" %><%@ include file="../methods/cookies.jspf" %><%@ include file="../methods/authusrs.jspf" %><%@ include file="../methods/nullif.jspf" %><%!
  ...
  public static int detectType(String sInput) {
...
%><% 
  ...
  if (!oTmp.canWrite()) {
    response.sendRedirect (response.encodeRedirectUrl ("../common/errmsg.jsp?title=SecurityException&desc=It is not possible to write at directory  "+sTmpDir+"&resume=_back"));  
    return;
  }
  ...
  try {
    oConn = GlobalDBBind.getConnection("userloader2");

    if (id_type.equals("USERS")) {
      oImpLoad = new UserLoader(oConn);
    } else if (id_type.equals("FELLOWS")) {
      oImpLoad = new FellowLoader(oConn);
    }
    ...
    oConn.close("userloader2");
  }
  catch (SQLException e) {  
    disposeConnection(oConn,"userloader2");
    oConn = null;
...
<HTML>
<HEAD>
  <TITLE>hipergate :: Load Users From File</TITLE>
...
<BODY  TOPMARGIN="8" MARGINHEIGHT="8">
  <FORM method="post" ACTION="userloader3.jsp" onsubmit="return validate()">
  ...
  <TABLE WIDTH="100%">
    ...
    <TR><TD CLASS="striptitle"><FONT CLASS="title1">Load Users From File</FONT></TD></TR>
  </TABLE>  
  <TABLE>
<%
      out.write("<TR>\n");
  ...
  <INPUT TYPE="button" class="pushbutton" VALUE="Previous" TITLE="ALT+b" ACCESSKEY="b" onclick="document.location='userloader2undo.jsp?action=_back&workarea=<%=gu_workarea%>&filename=<%=oTxtFile.getName()%>'">
  &nbsp;&nbsp;&nbsp;
  <INPUT TYPE="submit" class="pushbutton" VALUE="Import" TITLE="ALT+i" ACCESSKEY="i">
  &nbsp;&nbsp;&nbsp;
  <INPUT TYPE="button" class="closebutton" VALUE="Cancel" TITLE="ALT+c" ACCESSKEY="c" onclick="document.location='userloader2undo.jsp?action=_close&workarea=<%=gu_workarea%>&filename=<%=oTxtFile.getName()%>'">
  </FORM>
</BODY>
</HTML>
...
<%@ include file="../methods/dbbind.jsp" %><%@ include file="../methods/cookies.jspf" %><%@ include file="../methods/authusrs.jspf" %><%@ include file="../methods/clientip.jspf" %><%
  ...
  try {
    oCon = GlobalDBBind.getConnection("job_retryatoms");
	  oStm = oCon.prepareStatement("UPDATE "+DB.k_job_atoms+" SET "+DB.id_status+"="+String.valueOf(Job.STATUS_PENDING)+","+DB.tx_log+"=NULL WHERE "+DB.gu_job+"='"+gu_job+"' AND "+DB.pg_atom+"=?");

    oCon.setAutoCommit (false);
	  DBCommand.executeUpdate(oCon, "UPDATE "+DB.k_jobs+" SET "+DB.id_status+"="+String.valueOf(Job.STATUS_PENDING)+" WHERE "+DB.gu_job+"='"+gu_job+"'");
    ...
    oCon.commit();
    oCon.close("job_retryatoms");
  } 
  catch (SQLException e) {
      disposeConnection(oCon,"job_retryatoms");
      oCon = null; 
  ...
  response.sendRedirect (response.encodeRedirectUrl ("job_viewatoms.jsp?gu_job="+gu_job+"&id_status="+String.valueOf(Job.STATUS_PENDING)));

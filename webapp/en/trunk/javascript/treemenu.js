...
var treeMenuOlderOpen = "folderopen_16x16.gif";
var treeMenuOlderClosed = "folderclosed_16x16.gif";
...
function treeMenuDisplayItem(item) {
  ...
  if (item.url != "")
    if (item.target.length>0)
      ...
      treeMenuDocument.write('<a href="' + item.url + '" onClick="' + cmd + '">');    
  else
  ...
  if (item.submenu) {
    if (expanded)    
      img = treeMenuOlderOpen;
    else
      img = treeMenuOlderClosed;
  }
  else {
    if (item.icon != "")
      img = item.icon;
